#' Modified from Rhea package (Over-Time-Serial-Group-Comparisons):
...
# For running different taxons automatically. Turn off with '#'
for (taxon in c("ASV", "Family", "Genus", "Phylum")) {
...
rhea.dir <- file.path(stats.dir, "Rhea")

#' Please give the input file name 
input_file <-file.path(stats.dir, "correlation_matrix_relative.xlsx")            #<--- CHANGE ACCORDINGLY !!!

#' The name of the dependent variable that the analysis will be performed on (different time points)
dependant_variable_name <- "Lung"
...
if(taxon %in% c("Genus", "Family", "Phylum")){
  #' The cutoff of relative abundance; all values below this cutoff will be zeroed (default cutoff is 0.5 %)
  abundance_cutoff <- 0.005
...
# Load the file containing the values to be analyzed (samples names in the first column)
original_table <- read.xlsx(input_file, sheet = taxon)
original_table <- original_table[original_table$Diagnosis %in% c("Benign", "NSCLC", "SCLC"),]
#original_table <- original_table[original_table$Diagnosis %in% c("NSCLC"),]
twolungs <- names(which(rowSums(table(original_table$Study_Nr, original_table$Lung)) == 2))
original_table <- original_table[original_table$Study_Nr %in% twolungs,]
...
for (i in dependant_variables_start:dim(input_table)[2])
{
  ...
  # If a pvalue is calculated out of the Friedman Test, the pairwise Wilcox Signed Rank Test will be computed as well 
   ...
   # if (pvalue <= sig.cutoff){
      ...
      #pair_pval_table <- rbind(pair_pval_table,data.frame( measure = my_name,name = "All",Group1 = " -",Group2 = "-"))
 ...
 # }
  ...
  # Check if at least one of the tests (Wilcoxon or Fisher) is significant 
  # If at least one of them is significant a plot will be generated
  if (fish_pvalue <= sig.cutoff || (pair_pval_table$pvalue <= sig.cutoff & !is.na(pair_pval_table$pvalue))) {
    ...
    if (fish_pvalue <= sig.cutoff) {
      if (!is.na(signif_fpairs[1,1])) {
        ...
        colnames(signif_fpairs) <-c("Species","Groups","P-value")
        fpvaltable[[x]] <- list()

        # Pavlue table for significant pairs
        fpvaltable[[x]] <- tableGrob(signif_fpairs[2:3],rows = NULL,theme = mytheme)
...
# Add a column with the corrected p-values 
all_pair_pval_table$corrected <- round(p.adjust(all_pair_pval_table$pvalue, method = "BH"),4)
...
prefix = paste(dependant_variable_name, taxon, strsplit(basename(input_file),"[.]")[[1]][1],sep="_")

# Make a directory name with independent variable name and date
newdir <- paste(rhea.dir, prefix, Sys.Date(), sep = "_")
...
write.table(input_table, paste(prefix,"-modified.txt", sep = ""), sep = "\t", col.names = NA, quote = FALSE)

# Table with the results of Friedman test 
write.table(df[,c(1,2,4)], paste(prefix,"-pvalues.tab", sep = ""), sep = "\t", col.names = NA, quote = FALSE)

# Table with the results of Wilcoxon Signed Rank Sum Test
write.table(all_pair_pval_table[,-2], paste(prefix,"-sign_pairs.tab", sep = ""), sep = "\t", col.names = NA, quote = FALSE)

# Table with the results of Fisher's Exact Test
write.table(Fdf, paste(prefix,"-FisherTestAll.tab", sep = ""), sep = "\t", col.names = NA, quote = FALSE)

# Table with the results of pairwise Fisher's Exact Test
write.table(all_pair_fpval_table[,-2], paste(prefix,"-FisherTestPairWise.tab", sep = ""), sep = "\t", col.names = NA, quote = FALSE)

# Input file for correlation script (6.Correlation)
Corr_input_table <- input_table[,-c(1:(dependant_variables_start-1))]
suppressWarnings (try(write.table(Corr_input_table, paste(prefix,"Corr_input_table.tab",sep=""), sep ="\t",col.names = NA, quote = FALSE), silent =TRUE))
