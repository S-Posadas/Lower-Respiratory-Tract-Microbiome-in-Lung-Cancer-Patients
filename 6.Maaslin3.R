...
maaslin.dir=file.path(res.dir, "Maaslin3")
dir.create(maaslin.dir)

# Change species names for visualozation in Maaslin
...
#### Agglomerate taxa ####
physeq_ph <- tax_glom(physeq_re, "Phylum")
taxa_names(physeq_ph) <- tax_table(physeq_ph)[,"Phylum"]
physeq_g <- tax_glom(physeq_re, "Genus")
...
#physeq_maas0 <- subset_samples(physeq_maas0, colSums(otu_table(physeq_maas0)) != 0)
...
volcano <- function(maas.dir = maas.dir, name = name){
  ...
  ggplot(maaslin_all, aes(x = coef, y = -log10(qval_individual))) +
    geom_point() +
...
mas_1 <- maaslin3(input_data = data,  input_metadata = metadata,  output = file.path(maas.dir, name),
...
physeq_maas = subset_samples(physeq_maas, sample_data(physeq_maas)$Synchronous.tumor == "No" | is.na(sample_data(physeq_maas)$Synchronous.tumor))
...
#### NSCLC - history of smoking + histology ####
physeq_maas <- physeq_maas0
...
name = "diseased_NSCLC_smoking_histology"
data <- data.frame(otu_table(physeq_maas))
all(colnames(data) == rownames(metadata))
mas_1 <- maaslin3(input_data = data,  input_metadata = metadata,  output = file.path(maas.dir, name),
                  formula = ~ History.of.smoking.y.n + Histology.NSCLC,
                  min_abundance = min_abund,  min_prevalence = min_prev,
                  normalization = "TSS",  transform = "LOG",
                  #  max_significance = 0.05,
                  correction = "BH")
...
name = "diseased_NSCLC_smoking_histology_batch"
data <- data.frame(otu_table(physeq_maas))
all(colnames(data) == rownames(metadata))
mas_1 <- maaslin3(input_data = data,  input_metadata = metadata,  output = file.path(maas.dir, name),
                  min_abundance = min_abund,  min_prevalence = min_prev,
                  normalization = "TSS",  transform = "LOG",  
                  fixed_effects = c("History.of.smoking.y.n", "Histology.NSCLC"),
                  random_effects = c("Batch"),
                  #  max_significance = 0.05,
                  correction = "BH")
volcano(maas.dir, name)
