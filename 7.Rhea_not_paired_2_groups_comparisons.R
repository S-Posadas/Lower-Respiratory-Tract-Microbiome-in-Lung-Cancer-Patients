...
original_table <- original_table[original_table$Lung == "Diseased",]
original_table <- original_table[original_table$Diagnosis %in% c("NSCLC"),]
#original_table <- original_table[!is.na(original_table$History.of.smoking.y.n),]
#original_table <- original_table[original_table$Histology.NSCLC %in% c("Adenocarcinoma", "Squamous cell carcinoma"),]
original_table <- original_table[!is.na(original_table$M),]
...
#  if (!is.nan(pvalue)){
  ...
  #}
    ...
    # if (fish_pvalue <= sig.cutoff || (pair_pval_table$pvalue <= sig.cutoff & !is.nan(pair_pval_table$pvalue))) {
...
prefix = paste(independant_variable_name, taxon, strsplit(basename(input_file),"[.]")[[1]][1],sep="_")
...
suppressWarnings (try(write.table(Corr_input_table, paste(prefix,"_","Corr_input_table.tab",sep=""), sep ="\t",col.names = NA, quote = FALSE), silent =TRUE))
